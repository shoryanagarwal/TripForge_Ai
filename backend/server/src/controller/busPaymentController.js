const BusPaymentService = require('../service/busPayment.js');
const busPaymentService = new BusPaymentService();

class BusPaymentController{

    async create(req,res){

        try{
            const payment = await busPaymentService.createPayment({
                busBookingId:req.body.busBookingId,
    ...
    async getPaymentById(req,res){
        try{
            ...
            const payment = await busPaymentService.getPaymentById(paymentId);
            res.status(200).json({
...
module.exports = BusPaymentController;